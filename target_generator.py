...
def dist_hom(args):
    ...
    xx = xx[ii]
    yy = yy[ii]
    xx = xx / np.cos(args.dec) + args.ra
    yy = yy + args.dec
    N = len(xx)
    ids = [ "{}{:06d}".format(args.sci_prefix, id) for id in np.arange( N ) ]
    tt = [args.obj_type] * N
    ...
    ee = [args.exp_time] * N
    cc = [Column(ids, name="ID", dtype=str), \
...
parser.add_argument("-p", "--sci_prefix", type=str, default="T",
                    help="Prefix for science target IDs.")
...
if args.plot:
    ...
    telescope = nf.Telescope(args.tel_ra, args.tel_ra, args.tel_pos_ang, args.tel_obs_time)
    ...
    plt.plot(np.real( tpos ), np.imag( tpos ), 'kx',markersize=1, alpha=0.8)
